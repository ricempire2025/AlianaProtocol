...
不再采用单纯的“时间挖矿”，而是基于**对协议资金池的贡献度**进行加权产出。为了控制通胀，采用**“阶段性减半”**模型。

#### A. 核心公式
$$ MintAmount = ContributionValue \times BaseRate \times EpochMultiplier $$

*   **ContributionValue (贡献值)**：
    *   `makeDeposit`: 1 USDT = 1.0 Point
    *   `compound`: 1 USDT = **1.5 Point** (复投权重更高)
*   **BaseRate (基准产出率)**：初始设定为 **0.1 ALI / Point** (即存 100 USDT 产出 10 ALI)。
*   **EpochMultiplier (周期衰减系数)**：每 **90天** 为一个 Epoch，每个 Epoch 衰减 10%。

#### B. 衰减周期表 (示例)
| 周期 (Epoch) | 时间范围 | 衰减系数 | 产出率 (ALI/100 USDT) | 说明 |
|---|---|---|---|---|
| Epoch 1 | Day 1-90 | 100% | 10.0 | 创世红利期 |
| Epoch 2 | Day 91-180 | 90% | 9.0 | 早期参与者 |
| Epoch 3 | Day 181-270 | 81% | 8.1 | 稳定增长期 |
| ... | ... | ... | ... | ... |
| Epoch 10 | Day 811-900 | 38.7% | 3.87 | 成熟期 |
